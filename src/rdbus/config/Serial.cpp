...
void from_json( const nlohmann::json& j, Serial& x )
{
    ...
    int baudRate = 0;
    tools::parseKeyValue( j, "baud_rate", baudRate, "No 'baud_rate' field present in 'serial' section!" );

    int stopBitsCount = 0;
    tools::parseKeyValue( j, "stop_bits_count", stopBitsCount, "No 'stop_bits_count' field present in 'serial' section!" );
    ...
    tools::throwIf( !path.starts_with( '/' ), "Absolute path is required for 'path' in 'serial' section!" );
