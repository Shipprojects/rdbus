...
TEST( TestModule, Valid )
{
    ...
    EXPECT_EQ( module.absoluteOffset, 12 );
...
{
    const auto path = testFilePath + "negative_instances_absolute_offset.json";
...
TEST( TestModule, NoAbsoluteOffset )
{
    const auto path = testFilePath + "no_instances_absolute_offset.json";
    ...
    EXPECT_EQ( module.name, "Sensors" );
    EXPECT_EQ( module.pollTimeMs, Module::Millis( 150 ) );
    EXPECT_EQ( module.absoluteOffset, 0 );
